    ...
    case "$model" in
        casam|cfe)
            if [ "$CALIBRATION_ALGORITHM" = "pso" ]; then
                JOB_NAME="ngsh_${model}"
            else
                JOB_NAME="ngsh_${CALIBRATION_ALGORITHM}_${model}"
            fi
            ...
            sbatch \
                ...
                --tmp="$TMP_DISK" \
                --time="$TIME_LIMIT" \
                --export=ALL,CALIBRATION_ALGORITHM="$CALIBRATION_ALGORITHM",MODEL="$model",NGSH_ROOT="$NGSH_ROOT",BASIN_CSV="$BASIN_CSV" \
                "$ARRAY_SCRIPT"
            ;;
        *)
