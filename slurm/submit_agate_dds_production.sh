...
# Agate convenience wrapper for a production-sized DDS calibration submission.
# Run from any Agate login/desktop shell:
#   bash /users/4/plafolle/infil_proj/NextGenSandboxHub/slurm/submit_agate_dds_production.sh
#
# Override any setting by exporting it before calling this script, for example:
#   MODELS=casam MAX_ARRAY_CONCURRENT=200 bash slurm/submit_agate_dds_production.sh
...
export MEM_PER_CPU="${MEM_PER_CPU:-20G}"
export TMP_DISK="${TMP_DISK:-20G}"
export TIME_LIMIT="${TIME_LIMIT:-4-0}"

# DDS controls. N_PARTICLES/CONF_PARTICLES only control the one particle
# workspace created by -conf; DDS itself uses N_ITERATIONS candidate updates.
export N_PARTICLES="${N_PARTICLES:-1}"
export CONF_PARTICLES="${CONF_PARTICLES:-1}"
export N_ITERATIONS="${N_ITERATIONS:-500}"
export MAX_PARTICLE_PROCS="${MAX_PARTICLE_PROCS:-1}"

# This throttle applies per model array. With MODELS=casam,cfe and
# CPUS_PER_TASK=1, MAX_ARRAY_CONCURRENT=100 can allocate up to about 200 cores
# total if both arrays run.
...
cat <<EOF
=== NGSH Agate DDS production submission ===
NGSH_ROOT:                    $NGSH_ROOT
BASIN_CSV:                    $BASIN_CSV
Gages:                        $N_GAGES
Models:                       $MODELS
...
DDS iterations:               $N_ITERATIONS
Conf particles:               $CONF_PARTICLES
